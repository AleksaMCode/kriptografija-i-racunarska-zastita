...
for i in {1..100}; do openssl rsa -in keys/kljuc$i.key -pubout -out keys/public$i.pem ; if [[ "$(diff keys/public$i.pem client-pubkey.pem)" == "" ]]; then echo "MATCH: key$i"; break; fi; rm keys/public$i.pem; done 2>/dev/null
#Dobijamo: "MATCH: key77"

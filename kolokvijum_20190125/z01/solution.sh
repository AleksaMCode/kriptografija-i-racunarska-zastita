#!/bin/bash
#izlaz.enc je base64 kodovan. Prvo sto radimo jeste da dekodujemo datoteku iz base64 formata.
mv izlaz.enc izlaz.enc.base64
openssl enc -d -base64 -in izlaz.enc.base64 -out izlaz.enc
mkdir keys && mv *.key keys/
#Preimenujmo imena prvih 9 kljuceva, npr. iz 01 u 1
for i in {1..9}; do mv keys/kljuc0$i.key keys/kljuc$i.key; done
#Kako je radjena asimetricna enkripcija, potrebno je da pronadjemo sve RSA kljuceve.
#Napravimo folder dsa u kojeg cemo smjestiti sve dsa kljuceve.
mkdir keys/dsa
for i in {1..27}; do if [[ "$(cat keys/kljuc$i.key | awk 'NR==1')" =~ "DSA" ]]; then mv keys/kljuc$i.key keys/dsa/kljuc$i.key; fi done 2>/dev/null
...
for i in {1..27}; do openssl enc -d -base64 -in keys/kljuc$i.key -out keys/kljuc$i.der; if [[ -s keys/kljuc$i.der ]]; then rm keys/kljuc$i.key; else rm keys/kljuc$i.der; fi; done 2>/dev/null
...
or i in {8..26}; do openssl rsautl -decrypt -in izlaz.enc -out ulaz$i.txt -inkey keys/kljuc$i.pem; [[ -s ulaz$i.txt ]] || rm ulaz$i.txt; done 2>/dev/null

...
#Izbacimo prvo privatni RSA kljuc iz pkcs#12 datoteke.
...
#Izbacimo CA sertifikat iz p12 datoke i izdvojimo njegov public key.
...
for i in {1..100}; do openssl rsa -in keys/kljuc$i.key -inform DER -out keys/kljuc$i.pem -outform PEM && rm keys/kljuc$i.key; done 2>/dev/null
#Svi dobijeni kljucevi su private. Prvo poredimo sa private.pem kljucem.
for i in {1..100}; do if [[ "$(diff keys/kljuc$i.pem private.pem | awk 'NR==1')" == "" ]]; then echo "MATCH: kljuc$i.key with private.pem (cert Prvi zahtjev key)"; break; fi; done 2>/dev/null
...
for i in {1..100}; do openssl rsa -in keys/kljuc$i.pem -inform PEM -pubout -out keys/public$i.pem; if [[ "$(diff keys/public$i.pem prvizahtjev-pubkey.pem | awk 'NR==1')" == "" ]]; then echo "MATCH: kljuc$i.pem with prvizahtjev-pubkey.pem (CA key)"; fi; rm keys/public$i.pem; done 2>/dev/null

#!/bin/bash
mkdir {potpisi,kljucevi} && mv *.key kljucevi/ ; mv *.sign potpisi/
...
for i in {1..31}; do openssl enc -d -base64 -in potpisi/potpis$i.sign -out potpisi/potpis$i.txt; done
#Za provjerumo koristimo potpise *.txt.
...
for i in {1..25}; do TEXT=$(cat kljucevi/kljuc$i.key | awk 'NR==1'); if [[ "$TEXT" =~ "RSA" ]]; then mv kljucevi/kljuc$i.key kljucevi/rsa/kljuc$i.key; else mv kljucevi/kljuc$i.key kljucevi/dsa/kljuc$i.key; fi; done 2>/dev/null 
...
for i in {1..25}; do openssl dsa -in kljucevi/dsa/kljuc$i.key -pubout -out kljucevi/public$i.key; done 2>/dev/null # Napomena: kljuc 19 je kriptovan. Kako je on jedini takav pretpostavimo da je on rjesenje zadatka. Provjerimo nasu pretpostavku i samim tim skratimo vrijeme potrebno da se uradi zadatak.
#Napisimo skriptu.

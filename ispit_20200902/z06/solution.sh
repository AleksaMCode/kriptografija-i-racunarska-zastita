#!/bin/bash
mkdir stores && mv store*.p12 stores/
#kljuc.key je public.
mkdir -p keys/public
#Izbacimo private keys iz svih p12 datoteka.
for i in {1..50}; do openssl pkcs12 -in stores/store$i.p12 -nocerts -out keys/key$i.pem -passin pass:sigurnost -passout pass:sigurnost; done
#Izbaciom iz dobijenih kljuceva public sa kojima cemo raditi poredjenje.
for i in {1..50}; do openssl rsa -in keys/key$i.pem -pubout -out keys/public/pub-key$i.pem -passin pass:sigurnost; done 2>/dev/null
#Napisimo skriptu koja ce da pronadje match, ispise poruku o kojoj se p12 datoteci radi i kopiraje je u direktorijum z06.
touch script.sh && chmod 777 script.sh
./script.sh
#Dobijamo poruku "Match found. Key is located in store39.p12 file".s
...
#Obavezno podesimo openssl.cnf datoteku!
#Kreirajmo CA tijelo.
openssl req -x509 -new -key private/private.key -out ca.pem -config openssl.cnf 

#Kreirajmo dva kljuca, dva zahtjeva i na kraju ih potpisimo.

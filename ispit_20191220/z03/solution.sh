...
#Kljucevi su u der formatu dati.
...
#Ostalo nam je 10 kljuceva koji su base64 kodovani.
for i in {10..100..10}; do mv keys/kljuc$i.key keys/kljuc$i.key.base64 && openssl enc -d -base64 -in keys/kljuc$i.key.base64 -out keys/kljuc$i.key; done; rm keys/*.base64
for i in {10..100..10}; do openssl rsa -in keys/kljuc$i.key -inform der -out keys/key$i.pem -outform pem && rm keys/kljuc$i.key; done 2>/dev/null
#Svi kljucevi su RSA private.
#Izbacimo javne kljuceve iz RSA privatnih.
for i in {1..100}; do openssl rsa -in keys/key$i.pem -pubout -out keys/public$i.pem; done; rm keys/key*; 2>/dev/null

#Izbacimo sertifikate iz .p12 datoteke.
openssl pkcs12 -in cert.p12 -nokeys -cacerts -out s2.pem -passin pass:sigurnost
...
#Nadjimo match.
for i in {1..100}; do for j in {1..2}; do if [[ "$(diff keys/public$i.pem s$j.public | awk 'NR==1')" == "" ]]; then echo "MATCH: key$i" && mv keys/public$i.pem . && break 2; fi; done; done
#Dobijamo: "MATCH: key70".
